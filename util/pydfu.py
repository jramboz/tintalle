...
# Modified by Jason Ramboz for use with PySide6 GUI reporting
def write_elements(elements, mass_erase_used, progress_callback=None, gui=False):
    ...
    for elem in elements:
        ...
        elem_size = size
        elem_addr = addr
        if progress_callback and not gui:
            progress_callback(elem_addr, 0, elem_size)
        while size > 0:
            write_size = size
                ...
                for segment in mem_layout:
                    if addr >= segment['addr'] and \
                       addr <= segment['last_addr']:
                        ...
                        break
            write_memory(addr, data[:write_size], progress_callback,
                         elem_addr, elem_size)
            data = data[write_size:]
            addr += write_size
            size -= write_size
            if progress_callback and not gui:
                progress_callback(elem_addr, addr - elem_addr, elem_size)
            elif progress_callback and gui:
                progress_callback.emit((addr-elem_addr)*100//elem_size)
...
def main():
    ...
    if args.path:
        ...
        write_elements(elements, args.mass_erase, progress_callback=cli_progress)
